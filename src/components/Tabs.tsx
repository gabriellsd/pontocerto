...
export function Tabs({ active, onChange }: TabsProps) {
  ...
  return (
    <div
      className="flex w-full gap-1 overflow-x-auto bg-slate-200/60 dark:bg-slate-800/60 p-1 rounded-xl mb-3 sm:mb-4"
      role="tablist"
    >
      {TABS.map(({ id, label, Icon }) => {
        const isActive = id === active;
        const base =
          'min-w-[44%] sm:min-w-0 flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-2.5 px-2 sm:px-3 text-xs sm:text-sm font-medium rounded-lg transition-all whitespace-nowrap';
